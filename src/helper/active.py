...
GLOBAL_RESULT_PATH = "/home/user/Documents/LLMGNN-master/data/aax"
...
def density_query(b, x_embed, n_cluster, train_mask, seed, data, device):
    ...
    # print(f"Inside density_query: num_samples={b}, device={device}")
...
def compute_hits_hub(data, num_nodes, device):
    hub_path = "{}/hits_hub_{}.pt".format(GLOBAL_RESULT_PATH, num_nodes)
    if osp.exists(hub_path):
        hub = torch.load(hub_path)
        return hub 
    else:
        edges = [(int(i), int(j)) for i, j in zip(data.edge_index[0], data.edge_index[1])]
        nodes = list(range(data.x.shape[0]))
        ...
        hub_tensor = torch.tensor(list(h.values()), dtype=data.x.dtype, device=device)
        torch.save(hub_tensor, hub_path)
        
        return hub_tensor

def compute_hits_authority(data, num_nodes, device):
    authority_path = "{}/hits_authority_{}.pt".format(GLOBAL_RESULT_PATH, num_nodes)
    if osp.exists(authority_path):
        authority = torch.load(authority_path)
        return authority 
    else:
        edges = [(int(i), int(j)) for i, j in zip(data.edge_index[0], data.edge_index[1])]
        nodes = list(range(data.x.shape[0]))
        ...
        # Convert to tensor and save
        authority_tensor = torch.tensor(list(a.values()), dtype=data.x.dtype, device=device)
        torch.save(authority_tensor, authority_path)
        
        return authority_tensor
...
def pg2_query_0(b, x, data, train_mask, seed, device):
    seed_everything(seed)
    num_nodes = x.shape[0]
    ...
    print("degree values",degree)
    ## get aax
    # aax = compute_norm_aax(x, data.edge_index, num_nodes)
    page = compute_pagerank(data, num_nodes, device)
    #page = compute_hits_hub(data, num_nodes, device)
    #page = compute_hits_authority(data, num_nodes, device)
    #page = compute_hits_authority(data, num_nodes, device) + compute_hits_hub(data, num_nodes, device)
    #page = compute_hits_authority(data, num_nodes, device) + compute_pagerank(data, num_nodes, device)
    ...
    # Get percentile
    percentile = (torch.arange(N, dtype=x.dtype, device=device) / N)
    id_sorted = page.argsort(descending=True)
    ...
    age_score = (alpha) * density + (beta) * page + 0.2 * degree
    #0.13, 0.67 0.2 filter_wrong --> 0.81

    #0.08, 0.72 --> 73.01 ± 0.44 0.71
    #0.09, 0.71 --> 74.29 ± 0.37  73
    #0.1, 0.7 0.2 -->  74.17 +_ 0.06  72

    #0.11 0.69 0.2  --> 74.45 +- 0.69 72
    #0.12 0.68 0.2  --> 74.41 +- 0.30 73
    #0.13 0.67 0.2 --> 73.17 +- 0.21 0.73
    #0.14 0.66 0.2 --> 73.98 +- 0.29 73
    #0.15 0.65 0.2 --> 74.50 ± 0.36 73 (ratio = 0.2)    
        #ratio = 0.15 75.21 ± 0.38 0.73  ratio =0.16 75.11 ± 0.44
        #ratio = 0.14 74.97 ± 0.38
    #0.16, 0.64 0.2 --> 72.90 ± 0.41 72
...
#\eta=015
#0.0, 0.8    75.71 ± 0.68
#0.01,0.79   74.96 ± 0.42
#0.02,0.78   76.08 ± 0.50
#0.03, 0.77  74.49 ± 0.28
#0.04, 0.76  74.99 ± 0.29
#0.05, 0.75  74.92 ± 0.51


#\eta=0.2
#0.11 0.69 0.2  --> 74.45 +- 0.69
#0.12 0.68 0.2  --> 74.41 +- 0.30 73
#0.13 0.67 0.2 --> 73.17 +- 0.21 0.73
#0.14 0.66 0.2 --> 73.98 +- 0.29 73
#0.15 0.65 0.2 --> 74.50 ± 0.36 73 
#0.16, 0.64 0.2 --> 72.90 ± 0.41 72

#citeseer
#0.07 0.73 65.40 ± 0.49
#0.08, 0.72 66.28 ± 0.65
#0.09, 0.71 67.40 ± 0.57
#0.1, 0.7 66.32 ± 0.56
#0.11, 0.69 64.01 ± 0.57
#0.12, 0.68 62.71 ± 0.26

#0.14 0.66 63.13 ± 0.37
#0.15 0.65 63.20 ± 0.18
#0.16, 0.64 62.39 ± 0.28


#python3 src/main_plt_1.py  --dataset cora --model_name GCN --data_format sbert --main_seed_num 5 --split active --output_intermediate 0 --no_val 1 --strategy pagerank2 --debug 1 --total_budget 140 --filter_strategy consistency --loss_type ce --second_filter conf+entropy --epochs 150 --debug_gt_label 0 --early_stop_start 150 --filter_all_wrong_labels 0 --oracle 1 --ratio 0.2 --alpha 0.15 --beta 0.65
...
def pg2_query(b, x, data, train_mask, seed, device):
    ...
    # Compute degree
    row_index = data.edge_index[0]
    degree = degree_cal(row_index, num_nodes=num_nodes, dtype=x.dtype)
    


    # Compute PageRank or other centrality measures
    page = compute_pagerank(data, num_nodes, device)
    # page = compute_hits_hub(data, num_nodes, device)
    # page = compute_hits_authority(data, num_nodes, device)
    # page = compute_hits_authority(data, num_nodes, device) + compute_hits_hub(data, num_nodes, device)
    ...
    # Load density data
    density_path = osp.join(GLOBAL_RESULT_PATH, f'density_x_{num_nodes}_{b}.pt')
    density = torch.load(density_path, map_location='cpu')
    if hasattr(data, 'drop_idx'):
        density[data.drop_idx] = 0

    # Filter out nodes with degree above the threshold
    degree_threshold = 0
    degree_mask = degree > degree_threshold

    # Apply mask to density, page, and degree
    filtered_density = density[degree_mask]
    filtered_page = page[degree_mask]
    filtered_degree = degree[degree_mask]


    # Check if filtered tensors are non-empty
    if filtered_density.numel() == 0:
        raise ValueError("Filtered density tensor is empty.")
    if filtered_page.numel() == 0:
        raise ValueError("Filtered page tensor is empty.")
    if filtered_degree.numel() == 0:
        raise ValueError("Filtered degree tensor is empty.")

    # Get percentile
    percentile = (torch.arange(filtered_page.size(0), dtype=x.dtype, device=device) / filtered_page.size(0))
    
    # Print percentile for debugging
    #print(f"Percentile: {percentile}")

    # Sort filtered tensors and assign percentiles
    id_sorted = filtered_page.argsort(descending=True)
    #print(f"Page Sorted Indices: {id_sorted}")
    filtered_page[id_sorted] = percentile

    id_sorted = filtered_density.argsort(descending=True)
    #print(f"Density Sorted Indices: {id_sorted}")
    filtered_density[id_sorted] = percentile

    id_sorted = filtered_degree.argsort(descending=False)
    #print(f"Degree Sorted Indices: {id_sorted}")
    filtered_degree[id_sorted] = percentile

    # Handle case where there are fewer nodes than b
    num_filtered_nodes = filtered_density.size(0)
    if num_filtered_nodes < b:
        print(f"Warning: Fewer nodes available ({num_filtered_nodes}) than the requested top-k ({b}).")
        b = num_filtered_nodes  # Adjust b to the number of available nodes

    # Calculate age_score
    alpha, beta = data.params['age']
    age_score = (alpha * filtered_density) + (beta * filtered_page) + 0.2 * filtered_degree


    # Select top b nodes based on age_score
    _, indices = torch.topk(age_score, k=b)

    # Get the original indices of the selected nodes
    indices = torch.nonzero(degree_mask, as_tuple=True)[0][indices]
