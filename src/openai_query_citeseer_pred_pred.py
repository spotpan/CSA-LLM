...
results_dir = 'citeseer_results_pred_pred'
...
for node in range(num_nodes):
    ...
    if len(predecessors) > 0:
        ...
        selected_pred = random.choice(predecessors)
        selected_pred_text = get_first_30_words(data['raw_texts'][selected_pred])
        summary += f", which is cited by the paper that '{selected_pred_text}'"

        # Get predecessors of the selected successor
        pred_of_pred, _ = get_neighbors(adjacency_matrix_csr, selected_pred)
        if len(pred_of_pred) > 0:
            selected_pred_of_pred = random.choice(pred_of_pred)
            random_pred_of_pred_text = get_first_30_words(data['raw_texts'][selected_pred_of_pred])
            summary += f" that is cited by the paper that '{random_pred_of_pred_text}'"
