...
results_dir = 'citeseer_results_succ_succ'
...
for node in range(num_nodes):
    ...
    if len(successors) > 0:
        ...
        _, succ_of_succ = get_neighbors(adjacency_matrix_csr, selected_succ)
        if len(succ_of_succ) > 0:
            selected_succ_of_succ = random.choice(succ_of_succ)
            random_succ_of_succ_text = get_first_30_words(data['raw_texts'][selected_succ_of_succ])
            summary += f" that cites the paper that '{random_succ_of_succ_text}'"
