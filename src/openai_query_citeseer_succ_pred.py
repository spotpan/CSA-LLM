...
results_dir = 'citeseer_results_succ_pred'
...
for node in range(num_nodes):
    ...
    if len(predecessors) > 0:
        ...
        summary += f", and it is cited by the paper that '{selected_pred_text}'"

        # Get predecessors of the selected successor
        _, succ_of_pred = get_neighbors(adjacency_matrix_csr, selected_pred)
        if len(succ_of_pred) > 0:
            selected_succ_of_pred = random.choice(succ_of_pred)
            random_succ_of_pred_text = get_first_30_words(data['raw_texts'][selected_succ_of_pred])
            summary += f" that cites the paper that '{random_succ_of_pred_text}'"
